...
(function () {
  ...
  theme.Variants = (function () {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function () {
        ...
        window.currentVariant = { ...variant, product_id: this.product_id };
        this._hideVariantNoneFound();
        if (this.enableHistoryState) {
          this._updateHistoryState(variant);
        }
      },
      _hideVariantNoneFound: function () {
        const node = this.container.querySelector(
          ".address-finder-pdp [data-variant-none-found]"
        );
        if (node) {
  ...
  theme.CartForm = (function () {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function (html) {
        ...
        if (theme.settings.progress_enable == true) {
          if (
            theme.settings.free_shipping &&
            theme.settings.discount == false
          ) {
            promo_activated =
              subtotal >= theme.settings.minimum_cartTotalFreeSF
                ? theme.settings.progress_text
                : theme.strings.cartDrawer_notification
                    .replace("[promo]", "FREE SHIPPING")
                    .replace(
                      "[discount]",
                      theme.Currency.formatMoney(
                        theme.settings.cartTotal_discount,
                        theme.settings.moneyFormat
                      )
                    )
                    .replace(".00", "")
                    .replace(
                      "[min_total]",
                      theme.Currency.formatMoney(
                        theme.settings.minimum_cartTotalFreeSF,
                        theme.settings.moneyFormat
                      )
                    )
                    .replace(".00", "")
                    .replace(
                      "[difference]",
                      theme.Currency.formatMoney(
                        theme.settings.minimum_cartTotalFreeSF - subtotal,
                        theme.settings.moneyFormat
                      )
                    )
                    .replace(".00", "");
          } else if (
            theme.settings.free_shipping == false &&
            theme.settings.discount
          ) {
            promo_activated =
              subtotal >= theme.settings.minimum_cartTotal
                ? theme.settings.discount_text
                : theme.strings.cartDrawer_notification
                    .replace(
                      "[promo]",
                      theme.Currency.formatMoney(
                        theme.settings.cartTotal_discount,
                        theme.settings.moneyFormat
                      ).replace(".00", "") + "  OFF Shipping Fee"
                    )
                    .replace(
                      "[difference]",
                      theme.Currency.formatMoney(
                        theme.settings.minimum_cartTotal - subtotal,
                        theme.settings.moneyFormat
                      )
                    )
                    .replace(".00", "");
          } else if (theme.settings.free_shipping && theme.settings.discount) {
            promo_activated =
              subtotal >= theme.settings.minimum_cartTotalFreeSF
                ? theme.settings.progress_text
                : subtotal >= theme.settings.minimum_cartTotal
          ...
          data = {
            promoActivatedTotalPrice:
              theme.settings.cartTotal_discount != 0
                ? subtotal >= theme.settings.minimum_cartTotal
                  ? theme.Currency.formatMoney(
                      subtotal - theme.settings.cartTotal_discount,
                      theme.settings.moneyFormat
                    )
                  : ""
                : "",
            promoActivatedDiscount:
              theme.settings.cartTotal_discount != 0
                ? subtotal >= theme.settings.minimum_cartTotal
                  ...
                  : ""
                : "",
            promoActivated: promo_activated,
            promoActivatedPriceClass:
              theme.settings.cartTotal_discount != 0
                ? subtotal >= theme.settings.minimum_cartTotal
                  ? "discounted-price"
                  : ""
                : "",
            promoActivatedTextClass:
              subtotal <= theme.settings.minimum_cartTotal
                ? "promo-disable"
                : "",
            freeShipping: theme.settings.free_shipping
              ? Math.floor(
                  (subtotal /
                    100 /
                    (theme.settings.minimum_cartTotalFreeSF / 100)) *
                    100
                ) >= 100
                ? 100
                : Math.floor(
              ...
              : Math.floor(
                  (subtotal / 100 / (theme.settings.minimum_cartTotal / 100)) *
                    100
                ) >= 100
              ? 100
              : Math.floor(
            ...
            barAnimation: theme.settings.free_shipping
              ? Math.floor(
                  (subtotal /
                    100 /
                    (theme.settings.minimum_cartTotalFreeSF / 100)) *
              ...
              ? "animated"
              : " ",
            drawer_link1: theme.settings.drawer_link1,
            drawer_link2: theme.settings.drawer_link2,
            drawer_text1: theme.settings.drawer_text1,
            drawer_text2: theme.settings.drawer_text2,
          };

          if (document.querySelector(".promo-text") != null) {
            document.querySelector(".promo-text").innerHTML =
              "<p>" +
              data.promoActivated +
              '<br><span class="shop-message"><a href="' +
              data.drawer_link1 +
              '">' +
              data.drawer_text1 +
              '</a> | <a href="' +
              ...
              '">' +
              data.drawer_text2 +
              '</a></span></p><div class="drawer-progBar" style="background-color: #cecece; display: block; height: 10px;"><span class="stripes ' +
              data.barAnimation +
              '"style="width: ' +
              data.freeShipping +
              '%; height: 100%;"></span><div class="percentage">' +
              data.freeShipping +
              "%</div></div>";
            // $('.promo-text').addClass(data.promoActivatedTextClass);
          }
        }
      },
          ...
          fetch(url)
            ...
            .then(
              function (html) {
                ...
                // Re-hook up collapsible box triggers
                theme.collapsibles.init(this.container);
                var addressFinderPDPNode = this.blocksHolder.querySelector(
                  ".address-finder-pdp"
                );
                if (theme.addressFinderPDP) {
                  new theme.addressFinderPDP({}, addressFinderPDPNode);
                }
                var addressFinderButton = this.blocksHolder.querySelector(
                  "[data-address-finder-open-btn]"
                );
                if (addressFinderButton) {
                  addressFinderButton.on("click.open-modal", function () {
                    window.addressFinderModal.open();
                  });
                }
  ...
  theme.RecentlyView = (function () {
    function RecentlyView(options = {}, element) {}
    return RecentlyView;
  })();
  /*============================================================================
