...
export class ContactInformationInit extends CustardModule {
  ...
  setup() {
    ...
    const getOptionItems = async function (items = []) {
      const baseUrl = `https://omniproxy-uat.goldenabc.com/api/endpoints/getinventorybyproductpartnooroptionvalue/call?AffiliateExternalID=`+ window.affiliate_id +`&SupplierFullNames=`+ window.supplier_fullname +`&ProcessingOptions=SearchByUPC%3Dfalse&${buildItemsParams(
        items
      )}`;
