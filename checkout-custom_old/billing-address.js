...
export class BillingAddress extends CustardModule {
  ...
  selector() {
    return '[data-billing-address]';
  }

  setup() {
    const selectors = {
      SECTION_CONTENT: ".section__content",
      COUNTRY_INPUT: "#checkout_billing_address_country",
      BRG_WRAPPER: '[data-address-field="address1"]',
      PROVINCE_WRRAPPER: '[data-address-field="province"]',
      CITY_WRAPPER: '[data-address-field="city"]',
      PROVINCE_SELECT: "#checkout_billing_address_province",
      CITY_SELECT: "#checkout_billing_address_city",
    ...
    const adjustHTML = () => {
      ...
      address2Wrapper
        ...
        .append(
          `<select placeholder="${window.translation.city_placholder}" class="field__input field__input--select" name="checkout[billing_address][address2]" id="custom-city-select"><option selected disabled>${window.translation.city_placholder}</option></select>${carretHtml}`
        );
      ...
      //   .addClass("field__input-wrapper--select")
      //   .append(
      //     `<select placeholder="${window.translation.zip_label}" class="field__input field__input--select" name="checkout[billing_address][zip]" id="custom-zip-select"><option selected disabled>${window.translation.zip_label}</option></select>${carretHtml}`
      //   );
      address1Wrapper
        .addClass("field--show-floating-label")
        .find(selectors.INPUT_WRAPPER)
        .addClass("field__input-wrapper--select")
        .append(
          `<select placeholder="${window.translation.barangay_placeholder}" class="field__input field__input--select" name="checkout[billing_address][address1]" id="custom-bangaray-select"><option disabled selected>${window.translation.barangay_placeholder}</option></select>${carretHtml}`
        );
    ...
    const $prov = document.querySelector("#checkout_billing_address_province");
    const $city = document.querySelector("#custom-city-select");
    const $brgy = document.querySelector("#custom-bangaray-select");
    const $zip = document.querySelector("#checkout_billing_address_zip");
    ...
    const populateCity = (json) => {
      ...
      if (window.currentAddress) {
        ...
        (retrievedAddress && retrievedAddress["addProvince"] === $prov.value) ||
        $prov.options[$prov.selectedIndex].text.toLowerCase() ===
          String(retrievedAddress.province_name).toLowerCase()
      ) {
        const _value = retrievedAddress["addCity"]
          ? String(retrievedAddress["addCity"])
          : String(retrievedAddress.city_name);
        $city.value = _value;
        if ($city.selectedIndex === -1) {
          $city.selectedIndex = 0;
        }
    ...
    const loadDefaultAddress = () => {
      ...
      if (
        ...
        !this.$('[name="checkout[billing_address][first_name]"]').val() &&
          this.$('[name="checkout[billing_address][first_name]"]').val(
            window.Customer.first_name
          );
        !this.$('[name="checkout[billing_address][last_name]"]').val() &&
          this.$('[name="checkout[billing_address][last_name]"]').val(
            window.Customer.last_name
          );
        !this.$('[name="checkout[billing_address][phone]"]').val() &&
          this.$('[name="checkout[billing_address][phone]"]').val(
    ...
    this.$("#checkout_billing_address_id").on("change", (evt) => {
      const $el = this.$(evt.target);
