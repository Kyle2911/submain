...
export class ShippingInformation extends CustardModule {
  ...
  setup() {
    if (window.maintenance_mode == false) {
      ...
      const baseUrl = `https://omniproxy-uat.goldenabc.com/api/endpoints/getinventorybyproductpartnooroptionvalue/call?AffiliateExternalID=`+ window.affiliate_id +`&SupplierFullNames=ALL&ProcessingOptions=SearchByUPC%3Dfalse&${buildItemsParams()}`;
